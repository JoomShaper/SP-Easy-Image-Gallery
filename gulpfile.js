...
const config = {
    ...
    com_package_name: 'com_speasyimagegallery_fullpackage_v2.1.0.zip',
...
const tasks = {
    ...
    admin: {
        src: [
            config.srcPath + 'administrator/components/com_' + config.componentName + '/**/*.{xml,php,js,css,jpg,png,webp,gif,ttf,otf,woff,woff2,svg,eot,sql,json}'
        ],
    ...
    if (plugins) {
        plugins.map(plugin => {
            let plg_task = taskDone => {
                src([plugin.path + '/' + plugin.name + '/**/*.{xml,php,js,css,jpg,png,webp,gif,ttf,otf,woff,woff2,svg,eot,json}',
